...
int runMassFitter(TString configFileName)
{
  ...
  TString reflectionType = config["reflectionType"].GetString();
  ...
  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {
    ...
    if (bkgFuncConfig[iPt] == 0) {
      ...
      bkgFunc[iPt] = HFInvMassFitter::ExpoGauss;
  ...
  TH1F* hMassSgnPrompt[nPtBins];
  TH1F* hMassSgnFD[nPtBins];
  ...
  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {
    if (!isMc) {
      ...
      if (enableRefl) {
        ...
        hMassSgnFD[iPt] = static_cast<TH1F*>(inputFileRefl->Get(fdHistoName[iPt].data()));
        hMassSgn[iPt] = static_cast<TH1F*>(hMassSgnFD[iPt]->Clone());
        hMassSgnPrompt[iPt] = static_cast<TH1F*>(inputFileRefl->Get(promptHistoName[iPt].data()));
        for (int binx = 1; binx <= hMassSgn[iPt]->GetNbinsX(); binx++)
        {
          double massSgn = hMassSgnPrompt[iPt]->GetBinContent(binx) + hMassSgnFD[iPt]->GetBinContent(binx);
          hMassSgn[iPt]->SetBinContent(binx, massSgn);
          double massError =  sqrt(pow(hMassSgnPrompt[iPt]->GetBinError(binx), 2) + pow(hMassSgnFD[iPt]->GetBinError(binx), 2));
          hMassSgn[iPt]->SetBinError(binx, massError);
        }
        cout << "Integral de hMassSgn  " << hMassSgn[iPt]->Integral(hMassSgn[iPt]->FindBin(massMin[iPt] * 1.0001), hMassSgn[iPt]->FindBin(massMax[iPt] * 0.999)) << endl;
        cout << "Integral de hMassSgnFD  " << hMassSgnFD[iPt]->Integral(hMassSgnFD[iPt]->FindBin(massMin[iPt] * 1.0001), hMassSgnFD[iPt]->FindBin(massMax[iPt] * 0.999)) << endl;
        cout << "Integral de hMassSgnPrompt  " << hMassSgnPrompt[iPt]->Integral(hMassSgnPrompt[iPt]->FindBin(massMin[iPt] * 1.0001), hMassSgnPrompt[iPt]->FindBin(massMax[iPt] * 0.999)) << endl;
        if (!hMassRefl[iPt]) {
  ...
  TH1F* hMassForSgnPrompt[nPtBins];
  TH1F* hMassForSgnFD[nPtBins];
  ...
  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {
    ...
    if (enableRefl) {
      ...
      hMassForSgnPrompt[iPt] =
        reinterpret_cast<TH1F*>(hMassSgnPrompt[iPt]->Rebin(nRebin[iPt]));
      hMassForSgnFD[iPt] =
        reinterpret_cast<TH1F*>(hMassSgnFD[iPt]->Rebin(nRebin[iPt]));
    }
    ...
    if (isMc) {
      ...
      if (enableRefl) {
        if (reflectionType == "All") {
          ...
          cout << "Integral de hMassForSgn   " << reflOverSgn << endl;
        } else if (reflectionType == "Prompt") {
          reflOverSgn = hMassForSgnPrompt[iPt]->Integral(hMassForSgnPrompt[iPt]->FindBin(massMin[iPt] * 1.0001), hMassForSgnPrompt[iPt]->FindBin(massMax[iPt] * 0.999));
          cout << "Integral de hMassForSgnPrompt   " << reflOverSgn << endl;
        } else if (reflectionType == "FD") {
          reflOverSgn = hMassForSgnFD[iPt]->Integral(hMassForSgnFD[iPt]->FindBin(massMin[iPt] * 1.0001), hMassForSgnFD[iPt]->FindBin(massMax[iPt] * 0.999));
          cout << "Integral de hMassForSgnFD   " << reflOverSgn << endl;
        } else {
          cerr << "ERROR: reflectionType not supported. Please choose All, Prompt or FD" << endl;
          return -1;
        }
        ...
        cout << "Integral de hMassForRefl  " << hMassForRefl[iPt]->Integral(hMassForRefl[iPt]->FindBin(massMin[iPt] * 1.0001), hMassForRefl[iPt]->FindBin(massMax[iPt] * 0.999)) << endl;
        cout << " reflOverSgn " << reflOverSgn << " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
