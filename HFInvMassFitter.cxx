...
void HFInvMassFitter::doFit(Bool_t draw)
{
  ...
  { // data
    ...
    if (mHistoTemplateRefl)
    {
      ...
      cout << "\n\n\n"
           << endl;
      cout << "Refl / Sgn: " << mReflOverSgn << endl;
      cout << "\n\n\n"
           << endl;
...
void HFInvMassFitter::fillWorkspace(RooWorkspace &workspace)
{
  ...
  // bkg poly1
  RooRealVar Param0("Param0", "Parameter of function", 0.);
  RooAbsPdf *bkgFuncPoly0 = new RooPolynomial("bkgFuncPoly0", "background fit function", mass, RooArgSet(Param0));
  workspace.import(*bkgFuncPoly0);
  // bkg poly2
  ...
  // exp ^ (poly2)
  RooRealVar ExpoPoly2Param1("ExpoPoly2Param1", "Parameter of Expo Poly2 function", -1., -5., 5.);
  RooRealVar ExpoPoly2Param2("ExpoPoly2Param2", "Parameter of Expo Poly2 function", 0.);
  RooRealVar ExpoPoly2Param3("ExpoPoly2Param3", "Parameter of Expo Poly2 function", mIntegralBkg);
  // RooFormulaVar FuncExpoPoly2("FuncExpoPoly2", "exp(ExpoPoly2Param*mass*mass)", RooArgList(mass, ExpoPoly2Param));
  // RooAbsPdf* bkgFuncExpoPoly1 = new RooExponential("bkgFuncExpoPoly1", "background fit function", mass, tau);
  RooAbsPdf *bkgFuncExpoGauss = new RooGenericPdf("bkgFuncExpoGauss", "bkgFuncExpoGauss", "1/ExpoPoly2Param3 * exp(ExpoPoly2Param1 * mass + ExpoPoly2Param2*mass*mass)", RooArgSet(mass, ExpoPoly2Param1, ExpoPoly2Param2, ExpoPoly2Param3));
  // RooRealIntegral normExpoGauss("normExpoGauss", "Normalization integral", bkgFuncExpoGauss, mass);
  // bkgFuncExpoGauss->normalize();
  // RooAbsPdf* bkgFuncExpoGauss = new RooProdPdf("bkgFuncExpoGauss", "bkgFuncExpoGauss", RooArgList(bkgFuncExpoPoly1, bkgFuncExpoPoly2));
  // RooAbsPdf* bkgFuncExpoGauss = new RooExponential("bkgFuncExpo", "background fit function", mass, tau);
  // RooAbsPdf* bkgFuncExpoGauss = new RooGenericPdf("bkgFuncExpoGauss", "bkgFuncExpoGauss", "std::exp(mass*tau + 0.*mass*mass)", RooArgSet(mass, tau));
  workspace.import(*bkgFuncExpoGauss);
  // pow * exp
  ...
  // signal pdf
  RooRealVar mean("mean", "mean for signal fit", mMass, 1.7, 1.9);
  if (mBoundMean)
  {
  ...
  RooRealVar sigma("sigma", "sigma for signal", mSigmaSgn, mSigmaSgn - 0.2, mSigmaSgn + 0.2);
  if (mFixedSigma)
  {
  ...
  RooRealVar sigmaDoubleGaus("sigmaDoubleGaus", "sigma2Gaus", mSigmaSgn, mSigmaSgn - 0.2, mSigmaSgn + 0.2);
  if (mBoundSigma)
  {
  ...
  RooRealVar meanRefl("meanRefl", "mean for reflection", mMass, 0.0, 2.15);
  if (mBoundReflMean)
  {
    meanRefl.setMax(mMassReflUpLimit);
    meanRefl.setMin(mMassReflLowLimit);
  }
  RooRealVar sigmaRefl("sigmaRefl", "sigma for reflection", 0.012, 0.0, 0.25);
...
void HFInvMassFitter::drawFit(TVirtualPad *pad, Int_t writeFitInfo)
{
  ...
  if (writeFitInfo > 0)
  {
    ...
    {
      textInfoRight->AddText(Form("mean(free) = %.4f #pm %.4f", mRooMeanSgn->getVal(), mRooMeanSgn->getError()));
    }
    if (mFixedSigma)
    {
    ...
    // mWorkspace->writeToFile("workspace_massFitter.root");
    RooRealVar *rooRawYield = new RooRealVar("rooRawYield", "rawYield", mRawYield, mRawYield, mRawYield);
    RooWorkspace wOut("wOut");
    RooArgSet varsToSave;
    varsToSave.add(*mRooMeanSgn);
    varsToSave.add(*mRooSigmaSgn);
    varsToSave.add(*mWorkspace->var("tau"));
    varsToSave.add(*mWorkspace->var("sigmaReflDoubleGaus"));
    varsToSave.add(*mWorkspace->var("meanReflDoubleGaus"));
    varsToSave.add(*mWorkspace->var("sigmaRefl"));
    varsToSave.add(*mWorkspace->var("meanRefl"));
    varsToSave.add(*mWorkspace->var("fracRefl"));
    varsToSave.add(*rooRawYield);
    RooDataSet dataToSave("dataToSave", "dataToSave", varsToSave);
    wOut.import(dataToSave);
    TFile fileOut("workspace_massFitter.root", "RECREATE");
    wOut.Write();
    cout << "\n\n     Values to save \n";
    cout << "Signal sigma: " << mRooSigmaSgn->getVal() << endl;
    cout << "Signal mean: " << mRooMeanSgn->getVal() << endl;
    cout << "Refl sigma: " << mWorkspace->var("sigmaRefl")->getVal() << endl;
    cout << "Refl mean: " << mWorkspace->var("meanRefl")->getVal() << endl;
    cout << "Refl double gauss sigma: " << mWorkspace->var("sigmaReflDoubleGaus")->getVal() << endl;
    cout << "Refl double gauss mean: " << mWorkspace->var("meanReflDoubleGaus")->getVal() << endl;
    cout << "tau: " << mWorkspace->var("tau")->getVal() << endl;
    cout << "Signal raw yield: " << rooRawYield->getVal() << endl;
    fileOut.Close();
  }
}
...
RooAbsPdf *HFInvMassFitter::createBackgroundFitFunction(RooWorkspace *workspace)
{
  ...
  case 6:
  {
    bkgPdf = workspace->pdf("bkgFuncExpoGauss");
    break;
  }
  case 7:
  { // MC
    bkgPdf = workspace->pdf("bkgFuncPoly0");
    break;
  }
  default:
    break;
...
void HFInvMassFitter::plotBkg(RooAbsPdf *pdf)
{
  ...
  case 5:
    ...
    break;
  case 6:
    pdf->plotOn(mInvMassFrame, Components("bkgFuncExpoPoly2"), Name("Bkg_c"), LineColor(kRed));
    break;
  case 7:
    pdf->plotOn(mInvMassFrame, Components("bkgFuncPoly0"), Name("Bkg_c"), LineColor(kWhite));
    break;
  default:
    break;
  }
