...
Double_t BifurcatedCB::ApproxErf(Double_t arg) const
{
 ...
 Double_t BifurcatedCB::evaluate() const
 {
   ...
   if ( t <= -absAlphaL )
   {
      double a = TMath::Power(nL/absAlphaL, nL) * TMath::Exp(-0.5*absAlphaL*absAlphaL) ;
      double b = nL/absAlphaL - absAlphaL ;
      result = a/TMath::Power(b-t, nL) ;
   }
   // Right side
   else if ( t >= fabs(alphaR) )
   {
      double a = TMath::Power(nR/absAlphaR, nR) * TMath::Exp(-0.5*absAlphaR*absAlphaR) ;
      double b = nR/absAlphaR - absAlphaR ;
      result = a/TMath::Power(b+t, nR) ;
   }
   // Gaussian center
   else
   {
...
Double_t BifurcatedCB::analyticalIntegral(Int_t code, const char* rangeName) const
{
   ...
   if ( tmin <= -absAlphaL ) // Tmin is in the left tail
   {
      ...
      {
          if (useLogL) return a * sig * ( TMath::Log(b-tmin) - TMath::Log(b-tmax) ) ;
          else         return a*sig/(1.0-nL) * (1.0/TMath::Power(b-tmin, nL-1.0) - 1.0/TMath::Power(b-tmax, nL-1.0)) ;
      }
      else if ( tmax > -absAlphaL && tmax <= absAlphaR )
      {
          // Range extends further than left tail, so integrate it all
          if (useLogL) result = a * sig * ( TMath::Log(b-tmin) - TMath::Log(nL/absAlphaL) ) ;
          else         result = a*sig/(1.0-nL) * (1.0/TMath::Power(b-tmin, nL-1.0) - 1.0/TMath::Power(nL/absAlphaL, nL-1.0)) ;
      ...
      else // We need to integrate the full gaussian part
      {
          ...
          double aR = TMath::Power(nR/absAlphaR, nR)*TMath::Exp(-0.5*absAlphaR*absAlphaR) ;
          double bR = nR/absAlphaR - absAlphaR ;
          if (useLogR) return (result + aR*sig*( log(bR+tmax) - log(nR/absAlphaR) )) ;
          else         return (result + aR*sig/(1.0-nR)*(1.0/TMath::Power(bR+tmax, nR-1.0)-1.0/TMath::Power(nR/absAlphaR, nR-1.0))) ;
      }
   }
   else if ( tmin > -absAlphaL && tmin <= absAlphaR ) // Tmin is in the Gaussian part
   {
      ...
      else
      {
          ...
          // Finally, the right tail
          double aR = TMath::Power(nL/absAlphaR, nR)*TMath::Exp(-0.5*absAlphaR*absAlphaR) ;
          double bR = nR/absAlphaR - absAlphaR ;
          if (useLogR) return (result + aR*sig*( log(bR+tmax) - log(nR/absAlphaR) )) ;
          else         return (result + aR*sig/(1.0-nR)*(1.0/TMath::Power(bR+tmax, nR-1.0)-1.0/TMath::Power(nR/absAlphaR, nR-1.0))) ;
   ...
   {
      double a = TMath::Power(nL/absAlphaR, nR)*TMath::Exp(-0.5*absAlphaR*absAlphaR) ;
      double b = nR/absAlphaR - absAlphaR ;
      if (useLogR) return a*sig*( log(b+tmax) - log(nR/absAlphaR) ) ;
      else         return a*sig/(1.0-nR)*(1.0/TMath::Power(b+tmax, nR-1.0)-1.0/TMath::Power(nR/absAlphaR, nR-1.0)) ;
   }
